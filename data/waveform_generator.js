...
class WaveformGenerator extends Stream{
    ...
    set_has_noise(has_noise){
        ...
        */
        ...
        else{throw TypeError('has_noise should be boolean, \'' + typeof(has_noise) +  '\' was passed')} //error
    }
        ...
        */
        ...
        for(let j = 0; j < batch_size; j++){
            ...
            for (let i = 0; i <= this.n_featu-1; i++){
                data[j][i] = multiplier_a * this.h_function[choice_a][i] + multiplier_b * this.h_function[choice_b][i] + rand.rand_gauss();
            };

            if(this.has_noise){
                for (let i = this.n_featu; i < this.n_num_features; i++){
                    data[j][i] = rand.rand_gauss();
                };
            };
